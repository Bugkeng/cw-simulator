...
LED_PIN = board.GP25    # Status LED (or board.LED for Pico W compatibility)
...
print("Dit paddle: GP15 → '[' key") ## <-- CHANGED
print("Dah paddle: GP10 → ']' key") ## <-- CHANGED
    ...
    while True:
        ...
        if (current_dit_state != last_dit_state and
            ...
            if not current_dit_state:  # Pressed (LOW)
                if not dit_pressed:
                    keyboard.press(Keycode.LEFT_BRACKET) ## <-- CHANGED
                    dit_pressed = True
                    print("Dit ON ([)")
            else:  # Released (HIGH)
                if dit_pressed:
                    keyboard.release(Keycode.LEFT_BRACKET) ## <-- CHANGED
                    dit_pressed = False
                    print("Dit OFF")
        ...
        if (current_dah_state != last_dah_state and
            ...
            if not current_dah_state:  # Pressed (LOW)
                if not dah_pressed:
                    keyboard.press(Keycode.RIGHT_BRACKET) ## <-- CHANGED
                    dah_pressed = True
                    print("Dah ON (])")
            else:  # Released (HIGH)
                if dah_pressed:
                    keyboard.release(Keycode.RIGHT_BRACKET) ## <-- CHANGED
                    dah_pressed = False
                    print("Dah OFF")
